...
- `vps-toolkit publish`：把最近一次体检报告渲染成 NodeSeek 帖子（机器画像 + 结论表 + 在线报告链接 + 规则模板评价），预览确认后发布
- 可选 `--ai` 接入任意 OpenAI 兼容端点生成"AI 评价"段（数据摘要发出前先预览确认）
- 可选 `--image` 附加截图（经论坛官方图床 nodeimage API）
- 凭据仅存本机 600 权限文件；每次发送必须人工确认；`--dry-run` 只渲染不触网
